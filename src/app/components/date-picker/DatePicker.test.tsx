...
test('applies min and max attributes', () => {
  render(<DatePicker label="Employment start date" min="2000-01-01" max="2026-09-08" />);
...
test('shows error message and sets aria-invalid', () => {
  render(<DatePicker label="Date of birth" error="You must be at least 18 years old" />);
  const input = screen.getByLabelText('Date of birth');
...
test('shows helper text when there is no error', () => {
  render(<DatePicker label="Employment start date" helperText="Cannot be a future date" />);
  expect(screen.getByText('Cannot be a future date')).toBeInTheDocument();
});

test('is disabled and does not accept input', () => {
  render(<DatePicker label="Date of birth" disabled defaultValue="1995-06-15" />);
