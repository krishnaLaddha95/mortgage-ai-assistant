...
test('shows helper text when there is no error', () => {
  render(<Input label="Monthly income" helperText="Enter your gross monthly income" />);
  expect(screen.getByText('Enter your gross monthly income')).toBeInTheDocument();
});
...
test('hides helper text when an error is present', () => {
  ...
  expect(screen.queryByText('Enter your gross monthly income')).not.toBeInTheDocument();
