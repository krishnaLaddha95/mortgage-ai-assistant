...
test('renders option description when provided', () => {
  render(
    <ControlledRadioGroup
      options={[{ value: 'joint', label: 'Joint application', description: 'You and a co-applicant' }]}
    />
  );
...
test('shows helper text when there is no error', () => {
  render(<ControlledRadioGroup helperText="Choose the option that applies to you" />);
  expect(screen.getByText('Choose the option that applies to you')).toBeInTheDocument();
});
